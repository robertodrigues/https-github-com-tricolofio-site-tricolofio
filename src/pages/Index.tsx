...
import { supabase } from "@/integrations/supabase/client";
 ...
 const Index = () => {
  ...
  const goBack = () => { if (step > 0) setStep(step - 1); else setStage("landing"); };
  ...
  return <main className="min-h-screen bg-[#fbfaf6] text-[#183c35] selection:bg-[#dcece2]">
    ...
    {stage === "landing" && <section className="hero-shell mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-5 pb-12 pt-28 lg:grid-cols-[1.05fr_.95fr] lg:px-10 lg:pt-20">
      <div className="relative z-[1] max-w-xl animate-rise">
        <div className="eyebrow"><span className="eyebrow-dot" /> Tricologia com acolhimento</div>
        <h1 className="mt-6 text-[2.9rem] font-medium leading-[1.04] tracking-[-.055em] sm:text-6xl lg:text-[5.25rem]">Sua queda de cabelo tem uma <em>causa.</em></h1>
        <p className="mt-7 max-w-lg text-lg leading-8 text-[#5b746d]">Descubra o que o seu padrão pode estar sinalizando em uma avaliação capilar gratuita, feita em cerca de 2 minutos.</p>
        <button onClick={startQuiz} className="primary-button mt-9 w-full sm:w-auto">Fazer minha avaliação gratuita <ArrowRight size={19} /></button>
        <div className="mt-8 flex items-center gap-3 text-sm text-[#67837a]"><div className="avatar-stack"><span>F</span><span>M</span><span>A</span></div><span><strong className="text-[#315e53]">+2.400 avaliações</strong><br />realizadas com cuidado</span></div>
      </div>
      <div className="hero-visual animate-float"><div className="organic-ring" /><div className="photo-card"><img src="https://images.unsplash.com/photo-1529139574466-a303027c1d8b?auto=format&fit=crop&w=900&q=85" alt="Mulher sorrindo em um ambiente claro" /><div className="photo-caption"><span className="caption-icon"><Sparkles size={14} /></span><span><strong>Cuidado que começa<br />com escuta.</strong></span></div></div><div className="leaf-note"><Leaf size={17} /><span>Um olhar para<br /><strong>o todo.</strong></span></div></div>
      <div className="hero-footer lg:col-span-2"><span>Uma primeira clareza para o seu próximo passo</span><span className="scroll-line" /></div>
    </section>}

    {stage === "quiz" && <section className="quiz-shell mx-auto flex min-h-screen max-w-3xl flex-col px-5 pb-10 pt-28 lg:px-10">
      <div className="mb-10 flex items-center gap-4"><button onClick={goBack} className="icon-button" aria-label="Voltar"><ArrowLeft size={18} /></button><div className="flex-1"><div className="mb-2 flex justify-between text-xs font-bold tracking-[.13em] text-[#6b8880] uppercase"><span>Pergunta {step + 1} de {questions.length}</span><span>{Math.round(((step + 1) / questions.length) * 100)}%</span></div><div className="progress-track"><div className="progress-fill" style={{ width: `${((step + 1) / questions.length) * 100}%` }} /></div></div></div>
      <div key={step} className="animate-rise flex flex-1 flex-col"><div className="question-icon"><CurrentIcon size={25} /></div><span className="eyebrow mt-7">{questions[step].eyebrow}</span><h2 className="mt-4 max-w-2xl text-4xl font-medium leading-[1.08] tracking-[-.045em] sm:text-5xl">{questions[step].title}</h2><p className="mt-4 text-[#71877f]">Escolha a opção que mais se aproxima do que você percebe.</p><div className="mt-10 grid gap-3">{questions[step].options.map((option, i) => <button key={option} onClick={() => choose(option)} className="option-button"><span className="option-number">0{i + 1}</span><span>{option}</span><ChevronRight className="ml-auto text-[#99b0a8]" size={19} /></button>)}</div><p className="mt-auto pt-12 text-center text-xs text-[#91a59f]">Suas respostas são usadas somente para montar sua orientação inicial.</p></div>
    </section>}
